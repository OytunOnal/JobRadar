...
That is one page instead of a dozen tabs. It is also an SEO repost, a ghost
listing or a three-year-old evergreen ad arriving **labelled as one**, rather
than costing you an evening. Most sources need no key at all, and your CV never
leaves the disk.
...
- **Visa answers come from governments, not vibes.** The full public sponsor
  ...
  in all, are matched by name, so a licensed employer ranks first and says so.
...
- **It is not built around one person.** Tracks, seniority appetite, languages
  and regions all live in a profile generated from your CV, so a junior data
...
tokens from company names. Each one is then **probe-validated live** against
the platform's own API before ingest trusts it.
...
That is a working radar. Everything else is in
**[docs/SETUP.md](docs/SETUP.md)**: filling the 53k-board company pool, what
each step does, the full command list, and upgrading without losing your
database.
...
few seconds. Kill it whenever you like. Every lane resumes from what the
...
- [job-hunter](https://github.com/girshovich/job-hunter), for the Common Crawl
  CDX approach to finding ATS boards, and for a slug-rejection rule I
  deliberately diverged from (`tests/discovery.test.ts` says why).
- [ai-job-search](https://github.com/MadsLorentzen/ai-job-search), for
  organising search queries by function rather than by title.
- [job-ops](https://github.com/DaKheera47/job-ops), for going at the API first
  and treating scraping as the fallback, which is how the sponsor registers are
  read.
